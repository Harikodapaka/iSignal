      ...
      {loading ? (
        ...
        <GlassCard p="xl" style={{ textAlign: 'center' }}>
          ...
          <Text size="sm" style={{ color: 'var(--text-muted)' }}>
            Log a few entries to see your trends here
          </Text>
        </GlassCard>
      ) : (
        <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
          {analytics.map((metric, i) => {
            ...
            const TrendIcon =
              metric.trend === 'up' ? IconTrendingUp : metric.trend === 'down' ? IconTrendingDown : IconMinus;
            const trendColor =
              metric.trend === 'up' ? 'var(--green)' : metric.trend === 'down' ? 'var(--red)' : 'var(--text-faint)';
            ...
            return (
              <GlassCard key={metric.metricKey} p="lg" accentColor={color} className={`fade-in fade-in-${(i % 4) + 1}`}>
