      ...
      {loading ? (
        ...
        <GlassCard p="xl" style={{ textAlign: 'center' }}>
          ...
          <Text size="sm" style={{ color: 'var(--text-muted)' }}>
            Head to Today and start logging — metrics appear automatically
          </Text>
        </GlassCard>
      ) : (
        <>
...
function MetricRow({
  ...
  return (
    <GlassCard p="md" accentColor={metric.pinned ? color : undefined} className={`fade-in fade-in-${(delay % 5) + 1}`}>
          ...
          <Box style={{ minWidth: 0 }}>
            ...
            <Group gap={5} mt={3} style={{ flexWrap: 'nowrap' }}>
              ...
              {metric.aggregation && metric.valueType === 'number' && (
                <Badge size="xs" variant="outline" color="violet" radius="sm">
                  {metric.aggregation}
                </Badge>
              )}
              <Text
              ...
              >
                {metric.frequencyScore}× logged
              </Text>
            </Group>
          </Box>
        </Group>
        <Group gap="xs" style={{ flexShrink: 0 }}>
          <ActionIcon variant="subtle" size="sm" radius="xl" onClick={onEdit} style={{ color: 'var(--text-muted)' }}>
            <IconPencil size={14} />
          </ActionIcon>
          <Switch
            checked={metric.pinned}
            onChange={(e) => onToggle(metric.metricKey, e.currentTarget.checked)}
            color="orange"
            size="sm"
          />
        </Group>
