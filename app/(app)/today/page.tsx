    ...
    <Group gap="xl" align="center" style={{ flexWrap: 'wrap' }}>
      ...
      <Box style={{ position: 'relative', width: 140, height: 140, flexShrink: 0 }}>
          ...
          {r.map((radius, i) => {
            ...
            return (
              <g key={i}>
                ...
                <circle cx="70" cy="70" r={radius} fill="none" stroke={colors[i]} strokeWidth="10"
                  strokeLinecap="round"
                  strokeDasharray={circ}
                  strokeDashoffset={circ * (1 - pct / 100)}
                  style={{ transition: `stroke-dashoffset 1.2s cubic-bezier(.4,0,.2,1) ${i * 0.12}s` }}
                />
              </g>
            )
          })}
        ...
        <Box style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center' }}>
          <Text fw={800} style={{ fontSize: 22, letterSpacing: '-0.04em', color: 'var(--text-primary)', lineHeight: 1 }}>
            {score}
          </Text>
          <Text size="xs" style={{ color: 'var(--text-muted)' }}>score</Text>
        </Box>
      </Box>
      ...
      <Stack gap={12}>
        {labels.map(({ val, unit, label, color }) => (
          <Group key={label} gap={10} align="center">
            ...
            <Text size="xs" style={{ width: 68, color: 'var(--text-secondary)' }}>{label}</Text>
            <Text fw={700} size="md" style={{ color, letterSpacing: '-0.02em' }}>{val}</Text>
            {unit && <Text size="xs" style={{ color: 'var(--text-muted)' }}>{unit}</Text>}
          </Group>
        ))}
      </Stack>
    </Group>
    ...
    <Stack gap="xl">
      ...
      <Group align="flex-start" gap="md" className="fade-in fade-in-2" style={{ flexWrap: 'wrap' }}>
        ...
        <GlassCard p="lg" style={{ flex: '0 0 auto' }}>
          <Text fw={700} size="sm" mb={4} style={{ color: 'var(--text-primary)' }}>Daily Rings</Text>
        ...
        <SimpleGrid cols={{ base: 2, sm: 2 }} spacing="sm" style={{ flex: 1, minWidth: 200 }}>
          <GlassCard p="md">
            <Text size="xs" fw={700} tt="uppercase" style={{ letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 8 }}>Best Streak</Text>
            <Group gap={6} align="center">
              <IconFlame size={20} color="var(--orange)" />
              <Text fw={800} style={{ fontSize: 26, letterSpacing: '-0.04em', color: 'var(--text-primary)' }}>{bestStreak}</Text>
              <Text size="xs" style={{ color: 'var(--text-muted)' }}>days</Text>
            </Group>
          </GlassCard>
          <GlassCard p="md">
            <Text size="xs" fw={700} tt="uppercase" style={{ letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 8 }}>Weekly Score</Text>
            <Text fw={800} style={{ fontSize: 26, letterSpacing: '-0.04em', color: 'var(--green)' }}>{weeklyScore}</Text>
          </GlassCard>
          <GlassCard p="md">
            <Text size="xs" fw={700} tt="uppercase" style={{ letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 8 }}>Logged Today</Text>
            <Group gap={4} align="baseline">
              <Text fw={800} style={{ fontSize: 26, letterSpacing: '-0.04em', color: 'var(--orange)' }}>{loggedToday}</Text>
              <Text size="sm" style={{ color: 'var(--text-muted)' }}>/ {totalPinned}</Text>
            </Group>
          </GlassCard>
          <GlassCard p="md">
            <Text size="xs" fw={700} tt="uppercase" style={{ letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 8 }}>Entries</Text>
            <Text fw={800} style={{ fontSize: 26, letterSpacing: '-0.04em', color: 'var(--blue)' }}>{events.length}</Text>
          </GlassCard>
        </SimpleGrid>
      </Group>
      ...
      {loadingA ? (
        ...
        <Box className="fade-in fade-in-3">
          ...
          <SimpleGrid cols={{ base: 1, sm: 2, lg: 4 }} spacing="sm">
            {analytics.map(a => <MetricCard key={a.metricKey} analytics={a} />)}
          </SimpleGrid>
        </Box>
      ) : (
        <GlassCard p="xl" style={{ textAlign: 'center' }}>
          <Text style={{ fontSize: 32, marginBottom: 8 }}>⚡</Text>
          <Text fw={600} mb={4} style={{ color: 'var(--text-primary)' }}>Start logging to see your metrics</Text>
          <Text size="sm" style={{ color: 'var(--text-muted)' }}>After 3 logs the metric auto-pins to your dashboard</Text>
        </GlassCard>
      )}
        ...
        <GlassCard p={0} style={{ overflow: 'hidden' }}>
          {events.length === 0 ? (
            <Box p="xl" style={{ textAlign: 'center' }}>
              <Text size="sm" style={{ color: 'var(--text-muted)' }}>No entries yet — start logging above!</Text>
            </Box>
          ) : events.map((event, i) => {
            const color = getMetricColor(event.metricKey)
            const emoji = getMetricEmoji(event.metricKey)
            return (
              <Group key={String(event._id)} justify="space-between" p="md" className="slide-in"
                style={{ borderBottom: i < events.length - 1 ? '1px solid var(--sep)' : 'none', animationDelay: `${i * 0.04}s` }}>
                <Group gap="sm">
                  <Box style={{ width: 7, height: 7, borderRadius: '50%', background: color, flexShrink: 0 }} />
                  <Box>
                    <Group gap={6}>
                      <Text size="sm">{emoji}</Text>
                      <Text size="sm" fw={600} style={{ color: 'var(--text-primary)' }}>{event.metricKey}</Text>
                      {event.tags?.map(tag => (
                        <Badge key={tag} size="xs" variant="light" color="gray" radius="sm">{tag}</Badge>
                      ))}
                    </Group>
                    <Text size="xs" ff="monospace" style={{ color: 'var(--text-muted)' }}>{event.rawText}</Text>
                  </Box>
                </Group>
                <Box style={{ textAlign: 'right' }}>
                  <Text size="sm" fw={700} style={{ color }}>{formatValue(event.value, event.unit, event.valueType)}</Text>
                  <Text size="xs" style={{ color: 'var(--text-muted)' }}>{formatTime(event.timestamp)}</Text>
                </Box>
              </Group>
            )
          })}
