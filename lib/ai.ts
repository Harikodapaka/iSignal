...
async function groq(prompt: string, maxTokens = 150, retryCount = 1): Promise<string> {
  ...
  if (!key) { console.error('[AI] GROQ_API_KEY is not set'); return '' }
...
[{ "metricKey": "known_key", "value": null_or_number, "unit": null_or_string, "confidence": 0.0 }]`,
      ...
      const parsed = parseJSON<{ metricKey: string; value: number | null; unit: string | null; confidence: number }[]>(text, [])
