...
export function findMetric(key: string) {
  ...
  let bestDist = threshold + 1, bestIdx = -1
  for (const { term, metricIdx } of ALL_TERMS) {
    ...
    if (dist < bestDist) { bestDist = dist; bestIdx = metricIdx }
  }
...
// ── Public API ────────────────────────────────────────────────────────────────
export function getMetricColor(key: string): string { return findMetric(key)?.color ?? '#636366' }
export function getMetricEmoji(key: string): string { return findMetric(key)?.emoji ?? '📊' }
...
export function parseLogInput(raw: string, userMetricKeys: string[] = []): LogInputParsed | null {
  ...
  for (const token of tokens) {
    if (/^\d+(\.\d+)?$/.test(token)) { value = parseFloat(token); continue }
    const numUnit = token.match(/^(\d+\.?\d*)([a-z]+)$/)
    if (numUnit) { value = parseFloat(numUnit[1]); unit = UNIT_MAP[numUnit[2]] ?? numUnit[2]; continue }
    if (UNIT_MAP[token]) { unit = UNIT_MAP[token]; continue }
