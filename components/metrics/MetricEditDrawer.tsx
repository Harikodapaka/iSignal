...
import {
    Drawer, Stack, TextInput, Group, Button, Text, Box,
    SegmentedControl, Divider, Badge, Radio,
} from '@mantine/core'
...
export function MetricEditDrawer({ metric, opened, onClose, onSaved }: Props) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (data.success) {
                notifications.show({ message: 'Metric updated', color: 'green', autoClose: 2000 })
                onSaved(data.data)
                onClose()
            } else {
    ...
    return (
        <Drawer
            ...
            title={
                <Group gap="xs">
                    <Text fw={700} style={{ color: 'var(--text-primary)' }}>Edit metric</Text>
                    {metric && (
            ...
            {metric && (
                <Stack gap="lg" pt="md">
                    ...
                    <TextInput
                        ...
                        styles={{
                            label: { color: 'var(--text-muted)', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em' },
                            input: { background: 'var(--card-bg)', border: '1px solid var(--card-border)', color: 'var(--text-primary)' },
                        }}
                    />

                    <Box>
                        <Text size="xs" fw={600} tt="uppercase" mb={6}
                            style={{ letterSpacing: '0.07em', color: 'var(--text-muted)' }}>
                            Value type
                        </Text>
                        <SegmentedControl
                            fullWidth
                    ...
                    {valueType === 'number' && (
                        <Box>
                            ...
                            <Radio.Group value={aggregation} onChange={v => setAggregation(v as typeof aggregation)}>
                                <Stack gap="xs">
                                    {AGG_DATA.map(opt => (
                                        <Radio.Card key={opt.value} value={opt.value} radius="md" p="sm"
                                            styles={{
                                                card: {
                                        ...
                                        >
                                            <Group wrap="nowrap" align="flex-start">
                                                ...
                                                <Box>
                                                    <Text size="sm" fw={600} style={{ color: 'var(--text-primary)' }}>{opt.label}</Text>
                                                    <Text size="xs" style={{ color: 'var(--text-muted)', marginTop: 2 }}>
                    ...
                    <Group>
                        <Button
                            variant="default"
                            onClick={onClose}
                            style={{ flex: 1, background: 'var(--card-bg)', borderColor: 'var(--card-border)', color: 'var(--text-primary)' }}
                        >
                            Cancel
                        </Button>
                        <Button
